/*
    SUBSCRIBE table — modernize DATE/TIME storage
    ===============================================
    Today: [DATE] char(10)  holding strings like '2026/09/22'
           ...
           submission time — same root cause already fixed on CONTACT
           and LOGIN)
    After:  [DATE] date
            [TIME] time(0)
    ...
    Take a backup of the SUBSCRIBE table (or the whole DB) before Step 3.
*/
...
ALTER TABLE [dbo].[SUBSCRIBE] ADD [DATE_NEW] date NULL;
ALTER TABLE [dbo].[SUBSCRIBE] ADD [TIME_NEW] time(0) NULL;
GO
...
ALTER TABLE [dbo].[SUBSCRIBE] ALTER COLUMN [DATE_NEW] date NOT NULL;
ALTER TABLE [dbo].[SUBSCRIBE] ALTER COLUMN [TIME_NEW] time(0) NOT NULL;
GO

ALTER TABLE [dbo].[SUBSCRIBE] DROP COLUMN [DATE];
ALTER TABLE [dbo].[SUBSCRIBE] DROP COLUMN [TIME];
GO

EXEC sp_rename 'dbo.SUBSCRIBE.DATE_NEW', 'DATE', 'COLUMN';
EXEC sp_rename 'dbo.SUBSCRIBE.TIME_NEW', 'TIME', 'COLUMN';
GO
...
WHERE TABLE_NAME = 'SUBSCRIBE' AND COLUMN_NAME IN ('DATE', 'TIME');
GO
